...
async function Main(){
 while (1 == 1){
   ...
   BipkiIncreaseL.textContent= (potricule*0.1*potricule_speed).toFixed(1).toString() + " B/S";
...
function BuyPotricule(){
 ...
 if (bipki >= potricule_price){
  ...
  PotriculeButton.textContent = potricule_price.toFixed(1);
  if (bipki < potricule_price){
...
function BuyPotriculeSpeed(){
 ...
 if (bipki >= 5**potricule_speed){
  ...
  potricule_speed*=2;
  PotriculeSpeedButton.textContent = 5**potricule_speed.toFixed(1);
  if (bipki < 5**potricule_speed){
